...
export type POSSale = {
  ...
  paymentMethod: string; cashReceived: number | null; changeGiven: number | null;
  soldById: number | null; soldByName: string | null;
...
export async function adminListPOSSales(limit?: number) {
  const qs = limit ? `?limit=${limit}` : "";
  return handle<{ sales: POSSale[]; count: number }>(await apiFetch(`/api/admin/pos/sales${qs}`, j()));
}
export async function adminGetPOSStats() {
  return handle<POSStats>(await apiFetch("/api/admin/pos/sales/stats", j()));
}
export async function adminCreatePOSSale(body: {
  customerId: string | null; customerName: string;
  items: Partial<POSSaleItem>[];
  subtotal: number; discountAmount: number; discountPct: number; total: number;
  paymentMethod: string; cashReceived: number | null; changeGiven: number | null;
  notes: string;
}) {
...
export type StockItem = {
  id: string; productId: number; productName: string; sku: string;
  quantity: number; costPrice: number; reorderLevel: number; updatedAt: string | null;
};
export async function adminListStock() {
  return handle<StockItem[]>(await apiFetch("/api/admin/pos/stock", j()));
}
export async function adminSetStock(productId: number, body: { quantity?: number; costPrice?: number; reorderLevel?: number }) {
  return handle<StockItem>(await apiFetch(`/api/admin/pos/stock/${productId}`, json(body, "PUT")));
}
export async function adminAdjustStock(productId: number, change: number, reason?: string) {
  return handle<{ quantityBefore: number; quantityAfter: number }>(
    await apiFetch(`/api/admin/pos/stock/${productId}/adjust`, json({ change, reason })),
  );
}
export async function adminSyncStock(products: Array<{ id: number; name: string; productNumber: string | null }>) {
  return handle<{ ok: true; synced: number }>(
    await apiFetch("/api/admin/pos/stock/sync-products", json({ products })),
  );
}

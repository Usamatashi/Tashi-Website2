...
export default function Products() {
  ...
  function handleAdd(p: DisplayProduct) {
    addItem({
      ...
      category: p.badge ?? null,
    });
    setJustAdded(p.key);
    window.setTimeout(() => {
      setJustAdded((cur) => (cur === p.key ? null : cur));
    }, 1500);
  }
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center gap-3 rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 text-sm text-brand-800">
            ...
            <span>
              Add items to your cart, then check out with Cash on Delivery, Easypaisa, or
              JazzCash. Need help?{" "}
          ...
          {loading ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {products.map((p) => (
                ...
                >
                  <div className="relative flex h-40 items-center justify-center overflow-hidden bg-gradient-to-br from-ink-100 to-ink-50 sm:h-44">
                    {p.imageUrl ? (
                      <img
                        src={p.imageUrl}
                        alt={p.name}
